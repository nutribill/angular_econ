...
@Component({
  selector: 'dimple-barchart',
  template: `<svg width="100%" height="800"></svg>`
})
export class DimpleBarchartVizComponent extends DimpleBaseViz  {
  ...
  protected customizeChart() {
    let xAxis = this.chart.addCategoryAxis("x", ["Year", "Country"]);
    xAxis.addOrderRule(this.countries);
    let yAxis = this.chart.addMeasureAxis("y", this.datasetLabel);
    let countrySeries = this.chart.addSeries("Country", dimple.plot.bar);
